...
describe('Query', () => {
  ...
  it('emits after stale time is up', async () => {
    ...
    const query = new Query(subscriber, storage, { key: storageKey, queryFn: mockFetch, staleTime: 100, staleCheckInterval: 50 });
    await query.run()
    await new Promise(resolve => setTimeout(resolve, 200))
    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ cycle: QueryCycle.ON_STALE }))
  })

  it('calls onCancel if cancelled', async () => {
    const query = new Query(subscriber, storage, { key: 'test', queryFn: mockFetch, onCancel: mockCancel });
    query.cancel()
  ...
  it('will call post-cache cycle emit if value is wrong in storage', async () => {
    ...
    const query = new Query(subscriber, storage, { key: storageKey, queryFn: mockFetch });
    await query.run()

    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ cycle: QueryCycle.POST_CACHE_POPULATION }))
  })
  ...
  it('won\'t call post-cache cycle emit if value is wrong in storage', async () => {
    ...
    expect(mockNext).not.toHaveBeenCalledWith(expect.objectContaining({ cycle: QueryCycle.POST_CACHE_POPULATION }))
  })
  ...
  it('emit error if fetch call fails', async () => {
    ...
    expect(mockNext).toHaveBeenCalledWith(expect.objectContaining({ isError: true }))
  })
  ...
  it('emit only 2 times when observing only data', async () => {
    ...
    const query = new Query(subscriber, storage, { key: storageKey, queryFn: mockFetch, observe: ['data'] });
    await query.run()
  ...
  it('emits the right values for a simple fetch', async () => {
    ...
    // Last call condition
    expect(mockNext).toHaveBeenNthCalledWith(1, expect.objectContaining({ 
      data: undefined,
      isIdle: true,
      isStale: true,
      isLoading: false,
    ...
    expect(mockNext).toHaveBeenNthCalledWith(2, expect.objectContaining({ 
      data: undefined,
      isIdle: false,
      isStale: true,
      isLoading: true,
    ...
    expect(mockNext).toHaveBeenNthCalledWith(3, expect.objectContaining({ 
  ...
  it('calls the query function only once if staleTime is set and multiple query with same key are made', async () => {
    ...
    const query1 = new Query(subscriber, storage, { key: storageKey, queryFn: mockFetch, staleTime: 1000 * 60 })
    const query2 = new Query(subscriber, storage, { key: storageKey, queryFn: mockFetch })
